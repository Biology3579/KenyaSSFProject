...
base_panel_theme <- theme_classic(base_family = font) +
  theme(
    ...
    strip.text       = element_text(size = 12, face = "bold", family = font,
                                    margin = margin(3, 0, 3, 0))
  )

y_order       <- c("Human pressure", "Chlorophyll-a", "Rugosity")
y_order_total <- c("Settlement gravity", "Chlorophyll-a", "Rugosity")

# 1. Total Biomass ----
total_biomass <- tribble(
  ~term,               ~beta,  ~lower,  ~upper,
  "Rugosity",           0.216,  0.056,   0.376,
  "Chlorophyll-a",     -0.125, -0.322,   0.072,
  "Settlement gravity",-0.251, -0.445,  -0.057
)

plot_total <- ggplot(total_biomass, aes(x = beta, y = term)) +
  geom_vline(xintercept = 0, linewidth = 0.5, linetype = "dashed", color = "grey80") +
  geom_errorbarh(aes(xmin = lower, xmax = upper, color = term),
                 height = 0.14, linewidth = 1.1) +
  geom_point(aes(color = term), shape = 16, size = 3.2) +
  scale_color_manual(values = pred_cols) +
  scale_y_discrete(limits = y_order_total) +
  x_scale +
...
scrapers <- tribble(
  ~term,             ~beta,  ~lower,  ~upper,
  "Rugosity",         0.143, -0.024,   0.351,
  "Chlorophyll-a",    0.204,  0.013,   0.396,
  "Human pressure",  -0.261, -0.475,  -0.047
...
piscivores <- tribble(
  ~term,             ~beta,  ~lower,  ~upper,
  "Rugosity",         0.152, -0.120,   0.424,
  "Chlorophyll-a",   -0.071, -0.386,   0.243,
  "Human pressure",  -0.287, -0.584,   0.010
...
browsers <- tribble(
  ...
  "Chlorophyll-a",   -0.192, -0.481,   0.097,  "supported",
...
plot_browsers <- ggplot(browsers, aes(x = beta, y = term)) +
  geom_vline(xintercept = 0, linewidth = 0.5, linetype = "dashed", color = "grey80") +
  geom_errorbarh(
    data = filter(browsers, !is.na(lower)),
    aes(xmin = lower, xmax = upper, color = term),
    height = 0.14, linewidth = 1.1) +
  geom_point(
    data = filter(browsers, !is.na(lower)),
    aes(color = term), shape = 16, size = 3.2) +
  geom_point(
    data = filter(browsers, is.na(lower)),
    aes(x = 0), shape = 16, color = "grey55", size = 3.2) +
  scale_color_manual(values = pred_cols) +
  scale_y_discrete(limits = y_order) +
  x_scale +
  facet_wrap(~panel) +
...
corallivores <- tribble(
  ~term,             ~beta,  ~lower,  ~upper,  ~status,
  "Rugosity",         0.108, -0.062,   0.278,  "supported",
  "Chlorophyll-a",    0.121, -0.062,   0.304,  "supported",
  "Human pressure",   0.000,    NA,      NA,   "unsupported"
...
plot_corallivores <- ggplot(corallivores, aes(x = beta, y = term)) +
  geom_vline(xintercept = 0, linewidth = 0.5, linetype = "dashed", color = "grey80") +
  geom_errorbarh(
    data = filter(corallivores, !is.na(lower)),
    aes(xmin = lower, xmax = upper, color = term),
    height = 0.14, linewidth = 1.1) +
  geom_point(
    data = filter(corallivores, !is.na(lower)),
...
plot_grazers <- ggplot(grazer_detritivores, aes(x = beta, y = term)) +
  geom_vline(xintercept = 0, linewidth = 0.5, linetype = "dashed", color = "grey80") +
  geom_errorbarh(
    data = filter(grazer_detritivores, !is.na(lower)),
    aes(xmin = lower, xmax = upper, color = term),
    height = 0.14, linewidth = 1.1) +
  geom_point(
    data = filter(grazer_detritivores, !is.na(lower)),
...
plot_excavators <- ggplot(large_excavators, aes(x = beta, y = term)) +
  geom_vline(xintercept = 0, linewidth = 0.5, linetype = "dashed", color = "grey80") +
  geom_errorbarh(
    data = filter(large_excavators, !is.na(lower)),
    aes(xmin = lower, xmax = upper, color = term),
    height = 0.14, linewidth = 1.1) +
  geom_point(
    data = filter(large_excavators, !is.na(lower)),
...
# Legend ----
legend_data <- tibble(
  label  = c("Rugosity", "Chlorophyll-a", "Human pressure", "Unsupported"),
  colour = c("#A0522D", "#4A7C59", "#6A5ACD", "grey55"),
  y      = 4:1
)

legend_plot <- ggplot(legend_data, aes(x = 0.2, y = y, color = colour)) +
  geom_point(shape = 16, size = 4.2) +
  geom_text(aes(x = 0.38, label = label),
            color = "grey15", hjust = 0, size = 4.0, family = font) +
  annotate("text", x = 0.2, y = 4.9, label = "Legend", hjust = 0,
           fontface = "bold", size = 5.0, family = font, color = "grey10") +
  scale_color_identity() +
  scale_x_continuous(limits = c(0.1, 2.0)) +
  scale_y_continuous(limits = c(0.5, 5.0)) +
...
panel_b <- (row1 / row2) + plot_layout(heights = c(1, 1))

top_row <- wrap_elements(full =
                           (plot_total | legend_plot) +
...
fig3 <- ggdraw(fig3_notags) +
  ...
  draw_label("B.  Functional group responses",
             x = 0.02, y = 0.625, hjust = 0, vjust = 1,
             fontface = "bold", size = 13, fontfamily = font)

fig3

ggsave("fig3_revised.pdf", fig3, width = 10, height = 7.5,
       units = "in", bg = "white", useDingbats = FALSE)
ggsave("fig3_revised.png", fig3, width = 10, height = 7.5,
