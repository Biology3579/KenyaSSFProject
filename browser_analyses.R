# ============================================================
#  BROWSER BIOMASS
#  Response:   Browser biomass (continuous, zero-inflated)
...
browser_transects <- fish_2009 %>%
  ...
  summarise(
    transect_browser_biomass = sum(
      ifelse(trophic_group %in% c("browsers"), tot_wt_g, 0),
      na.rm = TRUE
    ),
...
cat("Transects after filtering:", nrow(browser_transects), "\n")
cat("Sites after filtering:",     n_distinct(browser_transects$site), "\n")


# ============================================================
#  BROWSER BIOMASS EXPLORATION
# ============================================================
...
browser_transects <- browser_transects %>%
  mutate(
    log_browser_biomass  = log(transect_browser_biomass + 0.01),
    sqrt_browser_biomass = sqrt(transect_browser_biomass)
  )

( browser_log <- ggplot(browser_transects, aes(x = log_browser_biomass)) +
    geom_histogram(bins = 30, fill = "#1a9641", colour = "white") +
    labs(x = "log(biomass + 0.01)", y = "Frequency",
         title = "Log-transformed Browser Biomass") +
    theme_bw() )

( browser_sqrt <- ggplot(browser_transects, aes(x = sqrt_browser_biomass)) +
    geom_histogram(bins = 30, fill = "#d7191c", colour = "white") +
    labs(x = "sqrt(biomass)", y = "Frequency",
         title = "Sqrt-transformed Browser Biomass") +
    theme_bw() )

jpeg("browser_biomass_distributions.jpg", width = 33, height = 11, units = "cm", res = 300)
...
# ── Variation by site ─────────────────────────────────────────
ggplot(browser_transects, aes(x = reorder(site, transect_browser_biomass, median),
...
browser_model_data <- browser_model_data %>%
  mutate(
...
predictor_vars <- browser_model_data %>%
  dplyr::select(
...
jpeg("dharma_F1_gaussian_browsers.jpg", width = 25, height = 15, units = "cm", res = 300)
...
jpeg("dharma_F2_tweedie_browsers.jpg", width = 25, height = 15, units = "cm", res = 300)
...
jpeg("dharma_F3_tweedie_zi_browsers.jpg", width = 25, height = 15, units = "cm", res = 300)
...
#  Response:   transect_browser_biomass (zero-inflated, continuous)
...
m0 <- glmmTMB(transect_browser_biomass ~ 1 + (1 | site),
              family = tweedie(link = "log"), data = browser_model_data)

m_env <- glmmTMB(transect_browser_biomass ~ sst_sc + log_chla_sc +
                   (1 | site),
                 family = tweedie(link = "log"), data = browser_model_data)

m_market <- glmmTMB(transect_browser_biomass ~ log_market_gravity_sc + (1 | site),
                    family = tweedie(link = "log"), data = browser_model_data)

m_settgrav <- glmmTMB(transect_browser_biomass ~ log_settlement_grav_sc + (1 | site),
                      family = tweedie(link = "log"), data = browser_model_data)

m_hab <- glmmTMB(transect_browser_biomass ~ rugosity_sc + (1 | site),
                 family = tweedie(link = "log"), data = browser_model_data)

m_env_market <- glmmTMB(transect_browser_biomass ~ sst_sc + log_chla_sc +
                          log_market_gravity_sc + (1 | site),
                        family = tweedie(link = "log"), data = browser_model_data)

m_env_settgrav <- glmmTMB(transect_browser_biomass ~ sst_sc + log_chla_sc +
                            log_settlement_grav_sc + (1 | site),
                          family = tweedie(link = "log"), data = browser_model_data)

m_habitat_market <- glmmTMB(transect_browser_biomass ~ rugosity_sc + log_market_gravity_sc + (1 | site),
                            family = tweedie(link = "log"), data = browser_model_data)

m_habitat_settgrav <- glmmTMB(transect_browser_biomass ~ rugosity_sc + log_settlement_grav_sc +(1 | site),
                              family = tweedie(link = "log"), data = browser_model_data)

m_full_market <- glmmTMB(transect_browser_biomass ~ sst_sc + log_chla_sc +
                           log_market_gravity_sc + rugosity_sc +
                           (1 | site),
                         family = tweedie(link = "log"), data = browser_model_data)

m_full_settgrav <- glmmTMB(transect_browser_biomass ~ sst_sc + log_chla_sc +
...
jpeg("dharma_browser_full_market_model.jpg", width = 25, height = 15, units = "cm", res = 300)
plot(res, main = "DHARMa diagnostics — browser best model")
dev.off()
...
( p_rug  <- plot_effect(m_hab, browser_model_data,
