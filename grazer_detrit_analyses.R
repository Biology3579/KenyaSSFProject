...
#  Response:   Grazer / detritivore biomass (continuous, zero-inflated)
...
# ── PREPARE GRAZER / DETRITIVORE MODEL DATA ─────────────────────────────────────────────────
...
# ============================================================
#  GRAZER / DETRITIVORE BIOMASS PER TRANSECT
# ============================================================
...
grazer_transects <- fish_2009 %>%
  ...
  summarise(
    transect_grazer_biomass = sum(
      ifelse(trophic_group %in% c("grazers", "detritivores", "grazer-detritivores"), tot_wt_g, 0),
      na.rm = TRUE
    ),
...
cat("Transects after filtering:", nrow(grazer_transects), "\n")
cat("Sites after filtering:",     n_distinct(grazer_transects$site), "\n")


# ============================================================
#  GRAZER / DETRITIVORE BIOMASS EXPLORATION
# ============================================================
...
jpeg("grazer_biomass_distributions.jpg", width = 33, height = 11, units = "cm", res = 300)
...
# ── Variation by site ─────────────────────────────────────────
ggplot(grazer_transects, aes(x = reorder(site, transect_grazer_biomass, median),
...
grazer_model_data <- grazer_model_data %>%
  mutate(
...
predictor_vars <- grazer_model_data %>%
  dplyr::select(
...
grazer_model_data <- grazer_model_data %>%
  mutate(log_grazer_biomass = log(transect_grazer_biomass + 1))
...
jpeg("dharma_F1_gaussian_grazers.jpg", width = 25, height = 15, units = "cm", res = 300)
...
mF2_tweedie <- glmmTMB(
  ...
  data   = grazer_model_data   # full data, zeros included
)
...
resF2_nested <- simulateResiduals(mF2_tweedie_nested, n = 1000)
plot(resF2_nested)
testDispersion(resF2_nested)
testZeroInflation(resF2_nested)
testOutliers(resF2_nested)

# ── AICc comparison ────────────────────────────────
aicc_family <- MuMIn::AICc(mF2_tweedie, mF2_tweedie_nested)
...
#  Response:   transect_grazer_biomass (zero-inflated, continuous)
...
m0 <- glmmTMB(transect_grazer_biomass ~ 1 + (1 | site),
              family = tweedie(link = "log"), data = grazer_model_data)

m_env <- glmmTMB(transect_grazer_biomass ~ sst_sc + log_chla_sc +
                   (1 | site),
                 family = tweedie(link = "log"), data = grazer_model_data)

m_market <- glmmTMB(transect_grazer_biomass ~ log_market_gravity_sc + (1 | site),
                    family = tweedie(link = "log"), data = grazer_model_data)

m_settgrav <- glmmTMB(transect_grazer_biomass ~ log_settlement_grav_sc + (1 | site),
                      family = tweedie(link = "log"), data = grazer_model_data)

m_hab <- glmmTMB(transect_grazer_biomass ~ rugosity_sc + (1 | site),
                 family = tweedie(link = "log"), data = grazer_model_data)

m_env_market <- glmmTMB(transect_grazer_biomass ~ sst_sc + log_chla_sc +
                          log_market_gravity_sc + (1 | site),
                        family = tweedie(link = "log"), data = grazer_model_data)

m_env_settgrav <- glmmTMB(transect_grazer_biomass ~ sst_sc + log_chla_sc +
                            log_settlement_grav_sc + (1 | site),
                          family = tweedie(link = "log"), data = grazer_model_data)

m_habitat_market <- glmmTMB(transect_grazer_biomass ~ rugosity_sc + log_market_gravity_sc + (1 | site),
                            family = tweedie(link = "log"), data = grazer_model_data)

m_habitat_settgrav <- glmmTMB(transect_grazer_biomass ~ rugosity_sc + log_settlement_grav_sc + (1 | site),
                              family = tweedie(link = "log"), data = grazer_model_data)

m_full_market <- glmmTMB(transect_grazer_biomass ~ sst_sc + log_chla_sc +
                           log_market_gravity_sc + rugosity_sc +
                           (1 | site),
                         family = tweedie(link = "log"), data = grazer_model_data)

m_full_settgrav <- glmmTMB(transect_grazer_biomass ~ sst_sc + log_chla_sc +
...
jpeg("dharma_grazer_full_market_model.jpg", width = 25, height = 15, units = "cm", res = 300)
plot(res, main = "DHARMa diagnostics — grazer / detritivore best model")
dev.off()
...
plotResiduals(res, grazer_model_data$sst_sc,                 xlab = "SST")
plotResiduals(res, grazer_model_data$log_chla_sc,            xlab = "Chl-a")
plotResiduals(res, grazer_model_data$rugosity_sc,            xlab = "Rugosity")
plotResiduals(res, grazer_model_data$log_market_gravity_sc,  xlab = "Gravity")
plotResiduals(res, grazer_model_data$site,                   xlab = "Site")
plotResiduals(res, grazer_model_data$country,                xlab = "Country")
...
p_sst  <- plot_effect(m_full_market, grazer_model_data,
                      "sst_sc",                "Sea surface temperature (scaled)")
p_chla <- plot_effect(m_full_market, grazer_model_data,
                      "log_chla_sc",           "Chlorophyll-a (scaled log)")
p_grav <- plot_effect(m_full_market, grazer_model_data,
                      "log_market_gravity_sc", "Market gravity (scaled log)")
p_rug  <- plot_effect(m_full_market, grazer_model_data,
                      "rugosity_sc",           "Rugosity (scaled)")

jpeg("effects_grazers.jpg", width = 35, height = 14, units = "cm", res = 300)
...
jpeg("dharma_tweedie_nested_grazers.jpg", width = 25, height = 15,
     units = "cm", res = 300)
...
plotResiduals(res_nested, grazer_model_data$country,               xlab = "Country")
plotResiduals(res_nested, grazer_model_data$sst_sc,                xlab = "SST")
plotResiduals(res_nested, grazer_model_data$log_chla_sc,           xlab = "Chl-a")
plotResiduals(res_nested, grazer_model_data$log_market_gravity_sc, xlab = "Market gravity")
plotResiduals(res_nested, grazer_model_data$rugosity_sc,           xlab = "Rugosity")

# ============================================================
#  CANDIDATE MODEL FITTING — NESTED RANDOM EFFECTS
# ============================================================

m0_nested <- glmmTMB(transect_grazer_biomass ~ 1 + (1 | country/site),
                     family = tweedie(link = "log"), data = grazer_model_data)

m_env_nested <- glmmTMB(transect_grazer_biomass ~ sst_sc + log_chla_sc +
                          (1 | country/site),
                        family = tweedie(link = "log"), data = grazer_model_data)

m_market_nested <- glmmTMB(transect_grazer_biomass ~ log_market_gravity_sc +
                             (1 | country/site),
                           family = tweedie(link = "log"), data = grazer_model_data)

m_settgrav_nested <- glmmTMB(transect_grazer_biomass ~ log_settlement_grav_sc +
                               (1 | country/site),
                             family = tweedie(link = "log"), data = grazer_model_data)

m_hab_nested <- glmmTMB(transect_grazer_biomass ~ rugosity_sc +
                          (1 | country/site),
                        family = tweedie(link = "log"), data = grazer_model_data)

m_env_market_nested <- glmmTMB(transect_grazer_biomass ~ sst_sc + log_chla_sc +
                                 log_market_gravity_sc + (1 | country/site),
                               family = tweedie(link = "log"), data = grazer_model_data)

m_env_settgrav_nested <- glmmTMB(transect_grazer_biomass ~ sst_sc + log_chla_sc +
                                   log_settlement_grav_sc + (1 | country/site),
                                 family = tweedie(link = "log"), data = grazer_model_data)

m_full_market_nested <- glmmTMB(transect_grazer_biomass ~ sst_sc + log_chla_sc +
                                  log_market_gravity_sc + rugosity_sc +
                                  (1 | country/site),
                                family = tweedie(link = "log"), data = grazer_model_data)

m_full_settgrav_nested <- glmmTMB(transect_grazer_biomass ~ sst_sc + log_chla_sc +
                                    log_settlement_grav_sc + rugosity_sc +
                                    (1 | country/site),
                                  family = tweedie(link = "log"), data = grazer_model_data)
...
delta_nested     <- aicc_vals_nested - min(aicc_vals_nested)
weights_nested   <- exp(-0.5 * delta_nested) / sum(exp(-0.5 * delta_nested))
...
# ============================================================
#  MARGINAL EFFECT PLOTS — NESTED MODEL
# ============================================================

p_sst_nested  <- plot_effect(mF2_tweedie_nested, grazer_model_data,
                             "sst_sc",                "Sea surface temperature (scaled)")
p_chla_nested <- plot_effect(mF2_tweedie_nested, grazer_model_data,
                             "log_chla_sc",           "Chlorophyll-a (scaled log)")
p_grav_nested <- plot_effect(mF2_tweedie_nested, grazer_model_data,
                             "log_market_gravity_sc", "Market gravity (scaled log)")
p_rug_nested  <- plot_effect(mF2_tweedie_nested, grazer_model_data,
                             "rugosity_sc",           "Rugosity (scaled)")

jpeg("effects_grazers_nested.jpg", width = 35, height = 14, units = "cm", res = 300)
